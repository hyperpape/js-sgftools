    ...
    for (var i = 1, ilen = boardHeight; i <= ilen; i++) {
        for (var j = 1; j <= boardWidth; j++) {
            if (stones[[j,i]]) {
                ...
                diagramText.push(Diagram.dChars[Diagram.EMPTY] + ' ');
            }
        }
        diagramText.push('\n');
    }
...
Fragment.coordinates = function (point) {
    return _.map(point.split(','), function (value) {
...
function ChildFragment (stones, parent, move) {
    ...
    this.position = this.positionList.newPosition(stones, this, parent);
}

ChildFragment.prototype.inTree = function () {
    return this.fragment.inTree;
};

ChildFragment.prototype.isInitial = function () {
    return this === this.fragment.children[0];
};

ChildFragment.prototype.placeMove = function () {
    return this.position.placeMove.apply(this.position, arguments);
};
